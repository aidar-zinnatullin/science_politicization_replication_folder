# # R script to test H3: Right-leaning outlets publish a higher daily share of COVID-19 coverage emphasizing broader 
# societal implications than centrist or left-leaning outlets.
...
n_articles_per_media <- corpus_covid_media %>% 
  group_by(Media) %>% 
  summarise(n_articles = n()) %>% 
  arrange(desc(n_articles)) %>% 
  ungroup() 

per_media_politicization <- soc_science_polit %>% 
  group_by(Media) %>% 
  summarise(n_articles = n()) %>% 
  arrange(desc(n_articles)) %>% 
  ungroup() 
...
table(corpus_covid_media$Media)
corpus_covid_media$ideology <- NA
corpus_covid_media$ideology[corpus_covid_media$Media == "Tagesschau"] <- "Centrist"
corpus_covid_media$ideology[corpus_covid_media$Media == "ZDF Heute"] <- "Centrist"
corpus_covid_media$ideology[corpus_covid_media$Media == "T-Online"] <- "Centrist"
corpus_covid_media$ideology[corpus_covid_media$Media == "RTL"] <- "Centrist"
corpus_covid_media$ideology[corpus_covid_media$Media == "NTV"] <- "Centrist"

corpus_covid_media$ideology[corpus_covid_media$Media == "Die Zeit"] <- "Center-Left"
corpus_covid_media$ideology[corpus_covid_media$Media == "Der Spiegel"] <- "Center-Left"
corpus_covid_media$ideology[corpus_covid_media$Media == "SZ"] <- "Center-Left"

corpus_covid_media$ideology[corpus_covid_media$Media == "Die Welt"] <- "Center-Right"
corpus_covid_media$ideology[corpus_covid_media$Media == "FAZ"] <- "Center-Right"
corpus_covid_media$ideology[corpus_covid_media$Media == "Bild"] <- "Center-Right"

table(corpus_covid_media$ideology)
32754 + 46947 + 77765 == nrow(corpus_covid_media)


# politicized
table(soc_science_polit$Media)
soc_science_polit$ideology <- NA
soc_science_polit$ideology[soc_science_polit$Media == "Tagesschau"] <- "Centrist"
soc_science_polit$ideology[soc_science_polit$Media == "ZDF Heute"] <- "Centrist"
soc_science_polit$ideology[soc_science_polit$Media == "T-Online"] <- "Centrist"
soc_science_polit$ideology[soc_science_polit$Media == "RTL"] <- "Centrist"
soc_science_polit$ideology[soc_science_polit$Media == "NTV"] <- "Centrist"

soc_science_polit$ideology[soc_science_polit$Media == "Die Zeit"] <- "Center-Left"
soc_science_polit$ideology[soc_science_polit$Media == "Der Spiegel"] <- "Center-Left"
soc_science_polit$ideology[soc_science_polit$Media == "SZ"] <- "Center-Left"

soc_science_polit$ideology[soc_science_polit$Media == "Die Welt"] <- "Center-Right"
soc_science_polit$ideology[soc_science_polit$Media == "FAZ"] <- "Center-Right"
soc_science_polit$ideology[soc_science_polit$Media == "Bild"] <- "Center-Right"

table(soc_science_polit$ideology)
231 + 417 + 123 == nrow(soc_science_polit)
corpus_covid_media$day <- as.Date(corpus_covid_media$Date)
soc_science_polit$day <- as.Date(soc_science_polit$Date)


# now I can summarize the corpus by media and day
corpus_covid_media_daily <-  corpus_covid_media %>% 
  group_by(Media, day) %>% 
  summarise(n_articles = n()) %>% 
  ungroup() 
...
# now I can summarize the corpus by media and day
soc_science_polit_daily <-  soc_science_polit %>% 
  group_by(Media, day) %>% 
  summarise(n_articles = n()) %>% 
  ungroup()
...
names(soc_science_polit_daily)
corpus_combined <- corpus_covid_media_daily %>% 
  left_join(soc_science_polit_daily, by = c("Media", "day")) %>% 
...
table(corpus_combined$ideology)
800 + 1872 + 2376 ==nrow(corpus_combined) # TRUE
...
table(data_long$ideology)
2166 + 2166 + 2888 ==nrow(data_long) # TRUE
...
data_long <- data_long %>%
  mutate(share_adj = ifelse(share == 0, epsilon, ifelse(share == 1, 1 - epsilon, share)))
data_long <- data_long %>%
  mutate(share_logit = log(share_adj / (1 - share_adj)))


lme_model_2 <- lmer(share_logit ~ ideology + (1|media) + (1|day), data = data_long)
summary(lme_model_2)
...
data_long <- data_long %>% left_join(cases_to_merge, by = "day")
...
coef_df <- as.data.frame(coef(summary(lme_model_2_cases))) %>%
...
tab5 <- tab5_df %>% 
  gt() %>% 
  tab_header(title = md("Table 4. Linear mixed-effects results: daily share of politicized non-biomedical COVID-19 coverage by outlet ideology.")) %>% 
  cols_align(align = "center", columns = -Variable)

gtsave(tab5, here("tables",  "table_5.html"))
