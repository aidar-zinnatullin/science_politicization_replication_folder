# R code to produce Appendix Tables 19, 20, and 21
...
corpus_combined <- smaller_corpus_daily %>% left_join(soc_science_polit_daily, by = c("Media", "day")) %>% 
...
tab19_df <- tibble(Component = c("Outlet (media)", "Day", "Residual"),
...
tab19 <- tab19_df %>% gt() %>% 
  tab_header(title = md("Table 19: Variance components and intraclass correlations (ICCs) for H1 model; linear mixed model fitted by REML with random intercepts for outlet and day, non-biomedical disciplines model.")) %>% 
  sub_missing(columns = "ICC", missing_text = "—") %>% 
  cols_align(align = "center", columns = -Component)

gtsave(tab19, here("tables", "tab_19_app.html"))
...
tab20 <- tab20_df %>%
  gt() %>%
  tab_header(title = md("Table 20: Fixed effects for H1 model (logit-transformed daily share of politicized content), non-biomedical disciplines model")) %>%
...
gtsave(tab20, here("tables", "tab_20_app.html"))
...
tab21_df <- tibble(Model = c("Day only", "Day + media"),
...
tab21 <- tab21_df %>% gt() %>% 
  tab_header(title = md("Table 21: Likelihood-ratio test comparing random-intercept specifications for H1 (models refit with maximum likelihood), non-biomedical disciplines model.")) %>% 
  sub_missing(columns = c("Chi-sq", "p-value"), missing_text = "—") %>% cols_align(align = "center", columns = -Model)

gtsave(tab21, here("tables", "tab_21_app.html"))
