# R script to replicate Figure 5 in Appendix D: Closeness of the media outlets by the share of politicized COVID-19 content in the all 
# COVID-related daily materials, text corpus with articles that contain covid-related keywords at least three times (non-biomedical disciplines).
...
soc_science_polit
soc_science_polit <- soc_science_polit %>%
  filter(!id_article %in% natural_science_polit$id_article)
...
jpeg("figures/fig_5_app.jpeg", width = 9, height = 6, units = 'in', res = 500)

plot(hc, main = "Hierarchical clustering of the politicized science \nreporting trends about COVID (non-biomedical disciplines)",
