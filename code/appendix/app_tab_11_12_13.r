# R code to produce Appendix Tables 11, 12, and 13
...
tab11_df <- tibble(Component = c("Outlet (media)", "Day", "Residual"),
...
tab11 <- tab11_df %>% gt() %>% 
  tab_header(title = md("Table 11: Variance components and intraclass correlations (ICCs) for H1 model; linear mixed model fitted by REML with random intercepts for outlet and day.")) %>% 
  sub_missing(columns = "ICC", missing_text = "—") %>% 
  cols_align(align = "center", columns = -Component)

gtsave(tab11, here("tables", "tab_11_app.html"))
...
tab12 <- tab12_df %>%
  gt() %>%
  tab_header(title = md("Table 12: Fixed effects for H1 model (logit-transformed daily share of politicized content), non-biomedical disciplines model")) %>%
...
gtsave(tab12, here("tables", "tab_12_app.html"))
...
tab13_df <- tibble(Model = c("Day only", "Day + media"),
...
tab13 <- tab13_df %>% gt() %>% 
  tab_header(title = md("Table 13: Likelihood-ratio test comparing random-intercept specifications for H1 (models refit with maximum likelihood)")) %>% 
  sub_missing(columns = c("Chi-sq", "p-value"), missing_text = "—") %>% cols_align(align = "center", columns = -Model)

gtsave(tab13, here("tables", "tab_13_app.html"))
